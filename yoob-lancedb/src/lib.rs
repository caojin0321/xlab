...
fn lancedb_to_yoob_error(e: lancedb::Error) -> VectorStoreError {
    VectorStoreError::DatastoreError(Box::new(e))
}

fn serde_to_yoob_error(e: serde_json::Error) -> VectorStoreError {
...
/// # Example
/// ```
/// use yoob_lancedb::{LanceDbVectorIndex, SearchParams};
/// use yoob::providers::openai::{Client, TEXT_EMBEDDING_ADA_002, EmbeddingModel};
///
...
/// # Example
/// ```
/// let search_params = yoob_lancedb::SearchParams::default().distance_type(lancedb::DistanceType::Cosine);
/// ```
    ...
    M: EmbeddingModel + Sync + Send,
{
    ...
    /// ```
    /// use yoob_lancedb::{LanceDbVectorIndex, SearchParams};
    /// use yoob::providers::openai::{EmbeddingModel, Client, TEXT_EMBEDDING_ADA_002};
    ///
    /// let openai_client = Client::from_env();
    ///
        ...
        self.build_query(query)
            ...
            .map(|(i, value)| {
                Ok((
                    ...
                    serde_json::from_value(value).map_err(serde_to_yoob_error)?,
                ))
            })
            .collect()
    }
