...
where
    M: EmbeddingModel,
{
    async fn insert_documents<Doc: serde::Serialize + xlab::Embed + Send>(
        &self,
        documents: Vec<(Doc, xlab::OneOrMany<xlab::embeddings::Embedding>)>,
    ) -> Result<(), xlab::vector_store::VectorStoreError> {
    ...
    M: EmbeddingModel,
{
    async fn top_n<T: for<'a> serde::Deserialize<'a> + Send>(
        ...
        if req.samples() > i32::MAX as u64 {
            return Err(VectorStoreError::DatastoreError(format!("The number of samples to return with the `xlab` AWS S3Vectors integration cannot be higher than {}", i32::MAX).into()));
        }
