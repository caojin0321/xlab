...
fn main() {
    let input_dir = "../xlab-core/src/providers";
...
use xlab::client::CompletionClient;
use crate::completion::{AssistantContent, CompletionRequest, Document, Message, ToolDefinition};
use xlab::completion::{Chat, CompletionModel, Prompt};
use crate::vector_store::JsVectorStore;
{% if module_name == "openai" %}
#[wasm_bindgen]
pub struct {{provider}}Agent(xlab::agent::Agent<xlab::providers::{{module_name}}::responses_api::ResponsesCompletionModel>);
{% else %}
#[wasm_bindgen]
pub struct {{provider}}Agent(xlab::agent::Agent<xlab::providers::{{module_name}}::CompletionModel>);
{% endif %}
...
impl {{provider}}Agent {
    ...
    pub fn new(opts: crate::JsAgentOpts) -> JsResult<Self> {
        ...
        let mut agent = xlab::providers::{{module_name}}::Client::new(&api_key).agent(&model);
    ...
    pub async fn chat(&self, prompt: &str, messages: Vec<Message>) -> JsResult<String> {
        let messages: Vec<xlab::message::Message> = messages
            .into_iter()
            .map(xlab::message::Message::from)
            .collect();
        self.0
...
pub struct OpenAIResponsesCompletionModel(
    xlab::providers::openai::responses_api::ResponsesCompletionModel,
);
...
impl OpenAIResponsesCompletionModel {
    ...
    pub fn new(opts: crate::JsModelOpts) -> JsResult<Self> {
        ...
        let client = xlab::providers::openai::Client::new(&model_opts.api_key);
    ...
    pub async fn completion(&self, opts: crate::JsCompletionOpts) -> JsResult<JsValue> {
        ...
        let req: xlab::completion::CompletionRequest = xlab::completion::CompletionRequest::from(req);
...
#[wasm_bindgen]
pub struct {{provider}}CompletionsCompletionModel(xlab::providers::openai::completion::CompletionModel);
...
impl {{provider}}CompletionsCompletionModel {
    ...
    pub fn new(opts: crate::JsModelOpts) -> JsResult<Self> {
        ...
        let client = xlab::providers::{{module_name}}::Client::new(&model_opts.api_key);
...
use xlab::embeddings::EmbeddingModel;
use xlab::client::embeddings::EmbeddingsClient;
#[wasm_bindgen]
pub struct {{provider}}EmbeddingModel(xlab::providers::{{module_name}}::embedding::EmbeddingModel);
...
use xlab::client::TranscriptionClient;
use xlab::transcription::{TranscriptionModel, TranscriptionResponse};
#[wasm_bindgen]
pub struct {{provider}}TranscriptionModel(xlab::providers::{{module_name}}::TranscriptionModel);
...
impl {{provider}}TranscriptionModel {
    ...
    pub async fn transcription(
        ...
        let req = xlab::transcription::TranscriptionRequest::from(req);
...
{% if image_gen %}
use xlab::client::image_generation::ImageGenerationClient;
use crate::image_generation::ImageGenerationRequest;
use xlab::image_generation::ImageGenerationModel;
#[wasm_bindgen]
pub struct {{provider}}ImageGenerationModel(xlab::providers::{{module_name}}::ImageGenerationModel);
...
impl {{provider}}ImageGenerationModel {
    ...
    pub async fn image_generation(
        ...
        let req = xlab::image_generation::ImageGenerationRequest::from(req);
        let res = self
            .0
...
pub struct {{provider}}ImageGenerationResponse(
    xlab::image_generation::ImageGenerationResponse<xlab::providers::{{module_name}}::ImageGenerationResponse>,
);
...
{% endif %}
{% if audio_gen %}
use xlab::client::AudioGenerationClient;
#[wasm_bindgen]
pub struct OpenAIAudioGenerationModel(
    xlab::providers::openai::audio_generation::AudioGenerationModel,
);
...
impl OpenAIAudioGenerationModel {
    ...
    pub async fn audio_generation(
        ...
        let req = xlab::audio_generation::AudioGenerationRequest::from(req);
        let res = self
            .0
            .audi(req)
            .await
