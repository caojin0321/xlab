...
fn main() {
    let input_dir = "../yoob-core/src/providers";
...
use yoob::streaming::StreamingPrompt;
use wasm_streams::ReadableStream;
use yoob::client::CompletionClient;
use crate::completion::{AssistantContent, CompletionRequest, Document, Message, ToolDefinition};
use yoob::completion::{Chat, CompletionModel, Prompt};
use crate::vector_store::JsVectorStore;
{% if module_name == "openai" %}
#[wasm_bindgen]
pub struct {{provider}}Agent(yoob::agent::Agent<yoob::providers::{{module_name}}::responses_api::ResponsesCompletionModel>);
{% else %}
#[wasm_bindgen]
pub struct {{provider}}Agent(yoob::agent::Agent<yoob::providers::{{module_name}}::CompletionModel>);
{% endif %}
...
impl {{provider}}Agent {
    ...
    pub fn new(opts: crate::JsAgentOpts) -> JsResult<Self> {
        ...
        let mut agent = yoob::providers::{{module_name}}::Client::new(&api_key).agent(&model);
    ...
    pub async fn chat(&self, prompt: &str, messages: Vec<Message>) -> JsResult<String> {
        let messages: Vec<yoob::message::Message> = messages
            .into_iter()
            .map(yoob::message::Message::from)
            .collect();
        self.0
...
pub struct OpenAIResponsesCompletionModel(
    yoob::providers::openai::responses_api::ResponsesCompletionModel,
);
...
impl OpenAIResponsesCompletionModel {
    ...
    pub fn new(opts: crate::JsModelOpts) -> JsResult<Self> {
        ...
        let client = yoob::providers::openai::Client::new(&model_opts.api_key);
    ...
    pub async fn completion(&self, opts: crate::JsCompletionOpts) -> JsResult<JsValue> {
        ...
        let req: yoob::completion::CompletionRequest = yoob::completion::CompletionRequest::from(req);
...
#[wasm_bindgen]
pub struct {{provider}}CompletionsCompletionModel(yoob::providers::openai::completion::CompletionModel);
...
impl {{provider}}CompletionsCompletionModel {
    ...
    pub fn new(opts: crate::JsModelOpts) -> JsResult<Self> {
        ...
        let client = yoob::providers::{{module_name}}::Client::new(&model_opts.api_key);
...
use yoob::embeddings::EmbeddingModel;
use yoob::client::embeddings::EmbeddingsClient;
#[wasm_bindgen]
pub struct {{provider}}EmbeddingModel(yoob::providers::{{module_name}}::embedding::EmbeddingModel);
...
use yoob::client::TranscriptionClient;
use yoob::transcription::{TranscriptionModel, TranscriptionResponse};
#[wasm_bindgen]
pub struct {{provider}}TranscriptionModel(yoob::providers::{{module_name}}::TranscriptionModel);
...
impl {{provider}}TranscriptionModel {
    ...
    pub async fn transcription(
        ...
        let req = yoob::transcription::TranscriptionRequest::from(req);
...
{% if image_gen %}
use yoob::client::image_generation::ImageGenerationClient;
use crate::image_generation::ImageGenerationRequest;
use yoob::image_generation::ImageGenerationModel;
#[wasm_bindgen]
pub struct {{provider}}ImageGenerationModel(yoob::providers::{{module_name}}::ImageGenerationModel);
...
impl {{provider}}ImageGenerationModel {
    ...
    pub async fn image_generation(
        ...
        let req = yoob::image_generation::ImageGenerationRequest::from(req);
        let res = self
            .0
...
pub struct {{provider}}ImageGenerationResponse(
    yoob::image_generation::ImageGenerationResponse<yoob::providers::{{module_name}}::ImageGenerationResponse>,
);
...
{% endif %}
{% if audio_gen %}
use yoob::client::AudioGenerationClient;
#[wasm_bindgen]
pub struct OpenAIAudioGenerationModel(
    yoob::providers::openai::audio_generation::AudioGenerationModel,
);
...
impl OpenAIAudioGenerationModel {
    ...
    pub async fn audio_generation(
        ...
        let req = yoob::audio_generation::AudioGenerationRequest::from(req);
        let res = self
            .0
            .audi(req)
            .await
