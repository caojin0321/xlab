...
#[wasm_bindgen]
pub struct GaladrielAgent(yoob::agent::Agent<yoob::providers::galadriel::CompletionModel>);
...
impl GaladrielAgent {
    ...
    pub fn new(opts: crate::JsAgentOpts) -> JsResult<Self> {
        ...
        let mut agent = if let Some(key) = fine_tune_key {
            yoob::providers::galadriel::Client::builder(&api_key)
                .fine_tune_api_key(&key)
                .build()
                .map_err(|x| JsError::new(x.to_string().as_ref()))?
        } else {
            yoob::providers::galadriel::Client::new(&api_key)
        }
        .agent(&model);
...
#[wasm_bindgen]
pub struct GaladrielCompletionsModel(yoob::providers::galadriel::CompletionModel);
...
impl GaladrielCompletionsModel {
    ...
    pub fn new(opts: crate::JsModelOpts) -> JsResult<Self> {
        ...
        let client = if let Some(key) = fine_tune_key {
            yoob::providers::galadriel::Client::builder(&model_opts.api_key)
                .fine_tune_api_key(key)
                .build()
                .map_err(|x| JsError::new(x.to_string().as_ref()))?
        } else {
            yoob::providers::galadriel::Client::new(&model_opts.api_key)
        };
