use xlab::vector_store::{VectorSearchRequest, VectorStoreError};
...
impl xlab::vector_store::VectorStoreIndex for JsVectorStore {
    fn top_n<T: for<'a> Deserialize<'a> + Send>(
        &self,
        req: VectorSearchRequest,
    ) -> impl std::future::Future<
        Output = Result<Vec<(f64, String, T)>, xlab::vector_store::VectorStoreError>,
    > + Send {
    ...
    fn top_n_ids(
        ...
        Output = Result<Vec<(f64, String)>, xlab::vector_store::VectorStoreError>,
    > + Send {
        let (result_tx, result_rx) = futures::channel::oneshot::channel();
        let (error_tx, error_rx) = futures::channel::oneshot::channel();
        let inner = self.inner.clone();
