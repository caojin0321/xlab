//! EternalAI API client and xlab integration
//!
//! # Example
//! ```
...
use xlab::client::ClientBuilderError;
use xlab::completion::GetTokenUsage;
use xlab::completion::{CompletionError, CompletionRequest};
use xlab::embeddings::{EmbeddingError, EmbeddingsBuilder};
use xlab::extractor::ExtractorBuilder;
use xlab::message;
use xlab::message::AssistantContent;
use xlab::providers::openai::{self, Message};
use xlab::streaming::{RawStreamingChoice, StreamingCompletionResponse};
use xlab::{Embed, completion, embeddings};
...
impl Client {
    ...
    /// # Example
    /// ```
    /// use xlab_eternalai::providers::eternalai::{ClientBuilder, self};
    ///
    ...
    /// # Example
    /// ```
    /// use xlab_eternalai::providers::eternalai::{Client, self};
    ///
    /// // Initialize the EternalAI client
    ...
    /// let eternalai = Client::new("your-open-ai-api-key");
    ///
    /// let embedding_model = eternalai.embedding_model("model-unknown-to-xlab", 3072);
    /// ```
...
impl GetTokenUsage for CompletionResponse {
    fn token_usage(&self) -> Option<xlab::completion::Usage> {
        let api_usage = self.usage.clone()?;
        let mut usage = xlab::completion::Usage::new();
...
impl completion::CompletionModel for CompletionModel {
    ...
    async fn completion(
        ...
        tracing::debug!(target: "xlab", "Sending completion request: {}", request);
