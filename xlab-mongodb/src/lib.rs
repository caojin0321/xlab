...
fn mongodb_to_xlab_error(e: mongodb::error::Error) -> VectorStoreError {
...
/// use xlab::{providers::openai, vector_store::VectorStoreIndex, client::{ProviderClient, EmbeddingsClient}};
///
    ...
    M: EmbeddingModel + Sync + Send,
{
    ...
    async fn top_n<T: for<'a> Deserialize<'a> + Send>(
        ...
        while let Some(doc) = cursor.next().await {
            let doc = doc.map_err(mongodb_to_xlab_error)?;
