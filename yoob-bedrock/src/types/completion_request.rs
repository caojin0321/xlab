...
use yoob::OneOrMany;
use yoob::completion::{CompletionError, Message};
use yoob::message::{ContentFormat, DocumentMediaType, UserContent};

pub struct AwsCompletionRequest(pub yoob::completion::CompletionRequest);
...
impl AwsCompletionRequest {
    ...
    pub fn messages(&self) -> Result<Vec<aws_bedrock::Message>, CompletionError> {
        ...
        full_history
            .into_iter()
            .map(|message| yoobMessage(message).try_into())
