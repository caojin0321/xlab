...
use super::{document::yoobDocument, image::yoobImage, tool::yoobToolResultContent};

pub struct yoobUserContent(pub UserContent);

impl TryFrom<aws_bedrock::ContentBlock> for yoobUserContent {
    type Error = CompletionError;

    fn try_from(value: aws_bedrock::ContentBlock) -> Result<Self, Self::Error> {
        match value {
            aws_bedrock::ContentBlock::Text(text) => {
                Ok(yoobUserContent(UserContent::Text(Text { text })))
            }
            aws_bedrock::ContentBlock::ToolResult(tool_result) => {
                let tool_result_contents = tool_result
                    ...
                    .collect::<Result<Vec<yoobToolResultContent>, _>>()?
                    .into_iter()
                    .map(|rt| rt.0)
                ...
                Ok(yoobUserContent(UserContent::ToolResult(ToolResult {
            ...
            aws_bedrock::ContentBlock::Document(document) => {
                let doc: yoobDocument = document.try_into()?;
                Ok(yoobUserContent(UserContent::Document(doc.0)))
            }
            aws_bedrock::ContentBlock::Image(image) => {
                let image: yoobImage = image.try_into()?;
                Ok(yoobUserContent(UserContent::Image(image.0)))
            }
...
impl TryFrom<yoobUserContent> for Vec<aws_bedrock::ContentBlock> {
    type Error = CompletionError;

    fn try_from(value: yoobUserContent) -> Result<Self, Self::Error> {
        match value.0 {
                ...
                let builder = aws_bedrock::ToolResultBlock::builder()
                    ...
                    .set_content(Some(
                        tool_result
                            .content
                            .into_iter()
                            .map(|tool| yoobToolResultContent(tool).try_into())
...
#[cfg(test)]
mod tests {
    use crate::types::user_content::yoobUserContent;
    use aws_sdk_bedrockruntime::types as aws_bedrock;
    use yoob::{
        OneOrMany,
        completion::CompletionError,
    ...
    fn aws_content_block_to_user_content() {
        ...
        let user_content: Result<yoobUserContent, _> = cb.try_into();
        assert!(user_content.is_ok());
        let content = match user_content.unwrap().0 {
            yoob::message::UserContent::Text(text) => Ok(text),
    ...
    fn aws_content_block_tool_to_user_content() {
        ...
        assert!(user_content.is_ok());
        let content = match user_content.unwrap().0 {
            yoob::message::UserContent::ToolResult(tool_result) => Ok(tool_result),
            _ => Err("Invalid content type"),
        };
        assert!(content.is_ok());
    ...
    fn user_content_to_aws_content_block() {
        let uc = yoobUserContent(UserContent::Text("txt".into()));
