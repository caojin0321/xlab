...
pub(crate) use crate::types::media_types::yoobDocumentMediaType;
use base64::{Engine, prelude::BASE64_STANDARD};
use uuid::Uuid;

#[derive(Clone)]
pub struct yoobDocument(pub Document);

impl TryFrom<yoobDocument> for aws_bedrock::DocumentBlock {
    type Error = CompletionError;

    fn try_from(value: yoobDocument) -> Result<Self, Self::Error> {
        let document_media_type = value
            .0
            .media_type
            .map(|doc| yoobDocumentMediaType(doc).try_into());
...
impl TryFrom<aws_bedrock::DocumentBlock> for yoobDocument {
    type Error = CompletionError;

    fn try_from(value: aws_bedrock::DocumentBlock) -> Result<Self, Self::Error> {
        let media_type: yoobDocumentMediaType = value.format.try_into()?;
...
mod tests {
    ...
    use crate::types::document::yoobDocument;

    #[test]
    fn test_document_to_aws_document() {
        let yoob_document = yoobDocument(Document {
            data: "data".into(),
            format: Some(ContentFormat::String),
            media_type: Some(DocumentMediaType::PDF),
            additional_params: None,
        });
        let aws_document: Result<aws_bedrock::DocumentBlock, _> = yoob_document.clone().try_into();
        ...
        let document_data = yoob_document.0.data.as_bytes().to_vec();
    ...
    fn test_base64_document_to_aws_document() {
        ...
        let aws_document: aws_bedrock::DocumentBlock = yoob_document.clone().try_into().unwrap();
        let document_data = BASE64_STANDARD.decode(yoob_document.0.data).unwrap();
        let aws_document_bytes = aws_document
            .source()
            .unwrap()
            .as_bytes()
    ...
    #[test]
    fn test_aws_document_to_yoob_document() {
        let data = aws_smithy_types::Blob::new("document_data");
        let document_source = aws_bedrock::DocumentSource::Bytes(data);
        let aws_document = aws_bedrock::DocumentBlock::builder()
            ...
            .build()
            .unwrap();
        let yoob_document: Result<yoobDocument, _> = aws_document.clone().try_into();
        assert!(yoob_document.is_ok());
        let yoob_document = yoob_document.unwrap().0;
        assert_eq!(yoob_document.media_type.unwrap(), DocumentMediaType::PDF)
    }

    #[test]
    fn test_unsupported_aws_document_to_yoob_document() {
        let data = aws_smithy_types::Blob::new("document_data");
        let document_source = aws_bedrock::DocumentSource::Bytes(data);
        let aws_document = aws_bedrock::DocumentBlock::builder()
            ...
            .unwrap();
        let yoob_document: Result<yoobDocument, _> = aws_document.clone().try_into();
        assert!(yoob_document.is_err());
        assert_eq!(
            yoob_document.err().unwrap().to_string(),
