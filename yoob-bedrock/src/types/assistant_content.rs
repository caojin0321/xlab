...
pub struct yoobAssistantContent(pub AssistantContent);

impl TryFrom<aws_bedrock::ContentBlock> for yoobAssistantContent {
    type Error = CompletionError;

    fn try_from(value: aws_bedrock::ContentBlock) -> Result<Self, Self::Error> {
        match value {
            aws_bedrock::ContentBlock::Text(text) => {
                Ok(yoobAssistantContent(AssistantContent::Text(Text { text })))
            }
            aws_bedrock::ContentBlock::ToolUse(call) => Ok(yoobAssistantContent(
...
impl TryFrom<yoobAssistantContent> for aws_bedrock::ContentBlock {
    type Error = CompletionError;

    fn try_from(value: yoobAssistantContent) -> Result<Self, Self::Error> {
        match value.0 {
...
mod tests {
    use crate::types::{
        assistant_content::yoobAssistantContent, converse_output::InternalConverseOutput,
        errors::TypeConversionError,
    };

    use super::AwsConverseOutput;
    use aws_sdk_bedrockruntime::types as aws_bedrock;
    use yoob::{OneOrMany, completion, message::AssistantContent};
    ...
    fn aws_content_block_to_assistant_content() {
        ...
        let yoob_assistant_content: Result<yoobAssistantContent, _> = content_block.try_into();
        assert!(yoob_assistant_content.is_ok());
        assert_eq!(
            yoob_assistant_content.unwrap().0,
