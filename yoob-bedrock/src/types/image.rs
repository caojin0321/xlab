...
#[derive(Clone)]
pub struct yoobImage(pub Image);

impl TryFrom<yoobImage> for aws_bedrock::ImageBlock {
    type Error = CompletionError;

    fn try_from(image: yoobImage) -> Result<Self, Self::Error> {
...
impl TryFrom<aws_bedrock::ImageBlock> for yoobImage {
    type Error = CompletionError;
...
mod tests {
    ...
    use crate::types::image::yoobImage;

    #[test]
    fn test_image_to_aws_image() {
        let yoob_image = yoobImage(Image {
        ...
        let aws_image: Result<aws_bedrock::ImageBlock, _> = yoob_image.clone().try_into();
        assert!(aws_image.is_ok());
        let aws_image = aws_image.unwrap();
        assert_eq!(aws_image.format, aws_bedrock::ImageFormat::Jpeg);
        let img_data = BASE64_STANDARD.decode(yoob_image.0.data).unwrap();
