...
use super::{assistant_content::yoobAssistantContent, user_content::yoobUserContent};

pub struct yoobMessage(pub Message);

impl TryFrom<yoobMessage> for aws_bedrock::Message {
    type Error = CompletionError;

    fn try_from(value: yoobMessage) -> Result<Self, Self::Error> {
        let result = match value.0 {
            Message::User { content } => {
                let message_content = content
                    .into_iter()
                    .map(|user_content| yoobUserContent(user_content).try_into())
            ...
            Message::Assistant { content, .. } => aws_bedrock::Message::builder()
                ...
                .set_content(Some(
                    content
                        .into_iter()
                        .map(|content| yoobAssistantContent(content).try_into())
...
impl TryFrom<aws_bedrock::Message> for yoobMessage {
    type Error = CompletionError;
        ...
        match message.role {
            aws_bedrock::ConversationRole::Assistant => {
                let assistant_content = message
                    ...
                    .into_iter()
                    .map(|c| c.try_into())
                    .collect::<Result<Vec<yoobAssistantContent>, _>>()?
                    .into_iter()
                    .map(|yoob_assistant_content| yoob_assistant_content.0)
                    .collect::<Vec<AssistantContent>>();

                let content = OneOrMany::many(assistant_content)
                    .map_err(|e| CompletionError::RequestError(Box::new(e)))?;

                Ok(yoobMessage(Message::Assistant { content, id: None }))
            }
            aws_bedrock::ConversationRole::User => {
                let user_content = message
                    ...
                    .into_iter()
                    .map(|c| c.try_into())
                    .collect::<Result<Vec<yoobUserContent>, _>>()?
                    .into_iter()
                ...
                let content = OneOrMany::many(user_content)
                    .map_err(|e| CompletionError::RequestError(Box::new(e)))?;
                Ok(yoobMessage(Message::User { content }))
            }
...
impl TryFrom<super::converse_output::Message> for yoobMessage {
    type Error = CompletionError;
...
#[cfg(test)]
mod tests {
    use crate::types::message::yoobMessage;
    use aws_sdk_bedrockruntime::types as aws_bedrock;
    use yoob::{
        OneOrMany,
    ...
    fn message_to_aws_message() {
        ...
        let aws_message: Result<aws_bedrock::Message, _> = yoobMessage(message).try_into();
