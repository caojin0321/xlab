use yoob::client::{CompletionClient, ProviderClient};
use yoob::{agent::AgentBuilder, completion::Prompt, loaders::FileLoader};
use yoob_bedrock::{client::Client, completion::AMAZON_NOVA_LITE};
use tracing::info;
...
async fn partial_agent() -> AgentBuilder<yoob_bedrock::completion::CompletionModel> {
    let client = client();
...
///
/// This example loads in all the rust examples from the yoob-core crate and uses them as\\
///  context for the agent
async fn loaders() -> Result<(), anyhow::Error> {
    let model = client().completion_model(AMAZON_NOVA_LITE);

    // Load in all the rust examples
    let examples = FileLoader::with_glob("yoob-core/examples/*.rs")?
        .read_with_path()
        .ignore_errors()
        .into_iter();
