//! EternalAI API client and yoob integration
//!
//! # Example
//! ```
...
use yoob::client::ClientBuilderError;
use yoob::completion::GetTokenUsage;
use yoob::completion::{CompletionError, CompletionRequest};
use yoob::embeddings::{EmbeddingError, EmbeddingsBuilder};
use yoob::extractor::ExtractorBuilder;
use yoob::message;
use yoob::message::AssistantContent;
use yoob::providers::openai::{self, Message};
use yoob::streaming::{RawStreamingChoice, StreamingCompletionResponse};
use yoob::{Embed, completion, embeddings};
...
impl Client {
    ...
    /// # Example
    /// ```
    /// use yoob_eternalai::providers::eternalai::{ClientBuilder, self};
    ///
    ...
    /// # Example
    /// ```
    /// use yoob_eternalai::providers::eternalai::{Client, self};
    ///
    /// // Initialize the EternalAI client
    ...
    /// let eternalai = Client::new("your-open-ai-api-key");
    ///
    /// let embedding_model = eternalai.embedding_model("model-unknown-to-yoob", 3072);
    /// ```
...
impl GetTokenUsage for CompletionResponse {
    fn token_usage(&self) -> Option<yoob::completion::Usage> {
        let api_usage = self.usage.clone()?;
        let mut usage = yoob::completion::Usage::new();
...
impl completion::CompletionModel for CompletionModel {
    ...
    async fn completion(
        ...
        tracing::debug!(target: "yoob", "Sending completion request: {}", request);
