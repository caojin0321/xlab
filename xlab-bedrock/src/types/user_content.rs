...
use super::{document::xlabDocument, image::xlabImage, tool::xlabToolResultContent};

pub struct xlabUserContent(pub UserContent);

impl TryFrom<aws_bedrock::ContentBlock> for xlabUserContent {
    type Error = CompletionError;

    fn try_from(value: aws_bedrock::ContentBlock) -> Result<Self, Self::Error> {
        match value {
            aws_bedrock::ContentBlock::Text(text) => {
                Ok(xlabUserContent(UserContent::Text(Text { text })))
            }
            aws_bedrock::ContentBlock::ToolResult(tool_result) => {
                let tool_result_contents = tool_result
                    ...
                    .collect::<Result<Vec<xlabToolResultContent>, _>>()?
                    .into_iter()
                    .map(|rt| rt.0)
                ...
                Ok(xlabUserContent(UserContent::ToolResult(ToolResult {
            ...
            aws_bedrock::ContentBlock::Document(document) => {
                let doc: xlabDocument = document.try_into()?;
                Ok(xlabUserContent(UserContent::Document(doc.0)))
            }
            aws_bedrock::ContentBlock::Image(image) => {
                let image: xlabImage = image.try_into()?;
                Ok(xlabUserContent(UserContent::Image(image.0)))
            }
...
impl TryFrom<xlabUserContent> for Vec<aws_bedrock::ContentBlock> {
    type Error = CompletionError;

    fn try_from(value: xlabUserContent) -> Result<Self, Self::Error> {
        match value.0 {
                ...
                let builder = aws_bedrock::ToolResultBlock::builder()
                    ...
                    .set_content(Some(
                        tool_result
                            .content
                            .into_iter()
                            .map(|tool| xlabToolResultContent(tool).try_into())
...
#[cfg(test)]
mod tests {
    use crate::types::user_content::xlabUserContent;
    use aws_sdk_bedrockruntime::types as aws_bedrock;
    use xlab::{
        OneOrMany,
        completion::CompletionError,
    ...
    fn aws_content_block_to_user_content() {
        ...
        let user_content: Result<xlabUserContent, _> = cb.try_into();
        assert!(user_content.is_ok());
        let content = match user_content.unwrap().0 {
            xlab::message::UserContent::Text(text) => Ok(text),
    ...
    fn aws_content_block_tool_to_user_content() {
        ...
        assert!(user_content.is_ok());
        let content = match user_content.unwrap().0 {
            xlab::message::UserContent::ToolResult(tool_result) => Ok(tool_result),
            _ => Err("Invalid content type"),
        };
        assert!(content.is_ok());
    ...
    fn user_content_to_aws_content_block() {
        let uc = xlabUserContent(UserContent::Text("txt".into()));
