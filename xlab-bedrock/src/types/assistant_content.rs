...
pub struct xlabAssistantContent(pub AssistantContent);

impl TryFrom<aws_bedrock::ContentBlock> for xlabAssistantContent {
    type Error = CompletionError;

    fn try_from(value: aws_bedrock::ContentBlock) -> Result<Self, Self::Error> {
        match value {
            aws_bedrock::ContentBlock::Text(text) => {
                Ok(xlabAssistantContent(AssistantContent::Text(Text { text })))
            }
            aws_bedrock::ContentBlock::ToolUse(call) => Ok(xlabAssistantContent(
...
impl TryFrom<xlabAssistantContent> for aws_bedrock::ContentBlock {
    type Error = CompletionError;

    fn try_from(value: xlabAssistantContent) -> Result<Self, Self::Error> {
        match value.0 {
...
mod tests {
    use crate::types::{
        assistant_content::xlabAssistantContent, converse_output::InternalConverseOutput,
        errors::TypeConversionError,
    };

    use super::AwsConverseOutput;
    use aws_sdk_bedrockruntime::types as aws_bedrock;
    use xlab::{OneOrMany, completion, message::AssistantContent};
    ...
    fn aws_content_block_to_assistant_content() {
        ...
        let xlab_assistant_content: Result<xlabAssistantContent, _> = content_block.try_into();
        assert!(xlab_assistant_content.is_ok());
        assert_eq!(
            xlab_assistant_content.unwrap().0,
