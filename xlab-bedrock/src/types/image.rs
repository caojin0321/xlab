...
#[derive(Clone)]
pub struct xlabImage(pub Image);

impl TryFrom<xlabImage> for aws_bedrock::ImageBlock {
    type Error = CompletionError;

    fn try_from(image: xlabImage) -> Result<Self, Self::Error> {
...
impl TryFrom<aws_bedrock::ImageBlock> for xlabImage {
    type Error = CompletionError;
...
mod tests {
    ...
    use crate::types::image::xlabImage;

    #[test]
    fn test_image_to_aws_image() {
        let xlab_image = xlabImage(Image {
        ...
        let aws_image: Result<aws_bedrock::ImageBlock, _> = xlab_image.clone().try_into();
        assert!(aws_image.is_ok());
        let aws_image = aws_image.unwrap();
        assert_eq!(aws_image.format, aws_bedrock::ImageFormat::Jpeg);
        let img_data = BASE64_STANDARD.decode(xlab_image.0.data).unwrap();
