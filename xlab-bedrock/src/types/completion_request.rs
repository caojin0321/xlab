...
use xlab::OneOrMany;
use xlab::completion::{CompletionError, Message};
use xlab::message::{ContentFormat, DocumentMediaType, UserContent};

pub struct AwsCompletionRequest(pub xlab::completion::CompletionRequest);
...
impl AwsCompletionRequest {
    ...
    pub fn messages(&self) -> Result<Vec<aws_bedrock::Message>, CompletionError> {
        ...
        full_history
            .into_iter()
            .map(|message| xlabMessage(message).try_into())
