...
pub(crate) use crate::types::media_types::xlabDocumentMediaType;
use base64::{Engine, prelude::BASE64_STANDARD};
use uuid::Uuid;

#[derive(Clone)]
pub struct xlabDocument(pub Document);

impl TryFrom<xlabDocument> for aws_bedrock::DocumentBlock {
    type Error = CompletionError;

    fn try_from(value: xlabDocument) -> Result<Self, Self::Error> {
        let document_media_type = value
            .0
            .media_type
            .map(|doc| xlabDocumentMediaType(doc).try_into());
...
impl TryFrom<aws_bedrock::DocumentBlock> for xlabDocument {
    type Error = CompletionError;

    fn try_from(value: aws_bedrock::DocumentBlock) -> Result<Self, Self::Error> {
        let media_type: xlabDocumentMediaType = value.format.try_into()?;
...
mod tests {
    ...
    use crate::types::document::xlabDocument;

    #[test]
    fn test_document_to_aws_document() {
        let xlab_document = xlabDocument(Document {
            data: "data".into(),
            format: Some(ContentFormat::String),
            media_type: Some(DocumentMediaType::PDF),
            additional_params: None,
        });
        let aws_document: Result<aws_bedrock::DocumentBlock, _> = xlab_document.clone().try_into();
        ...
        let document_data = xlab_document.0.data.as_bytes().to_vec();
    ...
    fn test_base64_document_to_aws_document() {
        ...
        let aws_document: aws_bedrock::DocumentBlock = xlab_document.clone().try_into().unwrap();
        let document_data = BASE64_STANDARD.decode(xlab_document.0.data).unwrap();
        let aws_document_bytes = aws_document
            .source()
            .unwrap()
            .as_bytes()
    ...
    #[test]
    fn test_aws_document_to_xlab_document() {
        let data = aws_smithy_types::Blob::new("document_data");
        let document_source = aws_bedrock::DocumentSource::Bytes(data);
        let aws_document = aws_bedrock::DocumentBlock::builder()
            ...
            .build()
            .unwrap();
        let xlab_document: Result<xlabDocument, _> = aws_document.clone().try_into();
        assert!(xlab_document.is_ok());
        let xlab_document = xlab_document.unwrap().0;
        assert_eq!(xlab_document.media_type.unwrap(), DocumentMediaType::PDF)
    }

    #[test]
    fn test_unsupported_aws_document_to_xlab_document() {
        let data = aws_smithy_types::Blob::new("document_data");
        let document_source = aws_bedrock::DocumentSource::Bytes(data);
        let aws_document = aws_bedrock::DocumentBlock::builder()
            ...
            .unwrap();
        let xlab_document: Result<xlabDocument, _> = aws_document.clone().try_into();
        assert!(xlab_document.is_err());
        assert_eq!(
            xlab_document.err().unwrap().to_string(),
