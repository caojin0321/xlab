...
use xlab::{agent::AgentBuilder, completion::Prompt, loaders::FileLoader};
...
async fn partial_agent() -> AgentBuilder<xlab_bedrock::completion::CompletionModel> {
    let client = client();
...
///
/// This example loads in all the rust examples from the xlab-core crate and uses them as\\
///  context for the agent
async fn loaders() -> Result<(), anyhow::Error> {
    let model = client().completion_model(AMAZON_NOVA_LITE);

    // Load in all the rust examples
    let examples = FileLoader::with_glob("xlab-core/examples/*.rs")?
        .read_with_path()
        .ignore_errors()
        .into_iter();
